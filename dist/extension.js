...
async function assign(context) {
    try {
        ...
        const enabled = cfgRoot.get('enable', true);
        if (!enabled)
            return;
        const lock = cfgRoot.get('lockColor') || '';
        ...
        if (!colorHex) {
            ...
            const s = cfgRoot.get('saturation', 0.6);
            const l = cfgRoot.get('lightness', 0.22);
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('workspaceTint.assign', () => assign(context)), vscode.commands.registerCommand('workspaceTint.clear', async () => {
        const cfg = vscode.workspace.getConfiguration();
        const current = cfg.get('workbench.colorCustomizations') || {};
        const keys = [
